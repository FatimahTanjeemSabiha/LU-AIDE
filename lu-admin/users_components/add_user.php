        ...
        if (move_uploaded_file($user_avater_loc, $avatar_path)) {
            ...
            if (!empty($user_name) && !empty($user_id) && !empty($user_password)) {
                if (!preg_match($phone_rgex, $user_phone)) {
                    ...
                    $query = "INSERT INTO `users`(`name`, `email`, `phone`, `st_id`, `department`, `dob`, `blood_group`, `password`, `profile`, `verification_code`, `is_verify`, `user_role`) VALUES ('$user_name','$user_email','$user_phone','$user_id','$user_department','$user_dob','$user_blood_group','$user_password','../$avatar_path','$verification_code','1','$user_role')";
